...
const[loading,setLoading]=useState(false);
  const getData = async () => {
    try {
      ...
      const filteredData = res.data.filter(item => item.name?.toLowerCase().includes(searchText.toLowerCase()) );
 ...
 <ActivityIndicator size="large" color="#0000ff" />
  ...
  {data.length === 0 ? (
    <View className="flex-1 justify-center ">
            <Text className="text-4xl text-center text-amber-400">No products found</Text>
            </View>
          ) :(    <FlatList
            data={data}
            renderItem={ItemView}
